...
class AStarPlannerAndControllerNode(Node):
    ...
    def map_callback(self, msg):
        if self.planning_complete: return
        self.planning_complete = True

        self.get_logger().info("Map received. Starting A* planning...")
        self.map_info = msg.info
        self.grid = np.array(msg.data).reshape((msg.info.height, msg.info.width))
        
        start_w = self.get_parameter('start_point').value
        goal_w = self.get_parameter('goal_point').value
        start_grid = self.world_to_grid(start_w)
